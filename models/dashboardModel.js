...
// =====================================================
// "TODAY", ANCHORED TO THE DATABASE'S OWN CLOCK
// =====================================================
// The app server (Vercel, running in UTC) and the MySQL server can be
// in different timezones. If "today" is computed with Node's `new
// Date()` while orders are stored/compared using MySQL's own NOW(),
// the two clocks can disagree on which calendar day it currently is —
// especially in the early-morning IST hours — which silently drops
// same-day earnings from Today/This Week/the 14-day graph until the
// mismatch rolls over. Anchoring "today" to the database's own clock
// keeps every comparison on one single source of truth.
let cachedDbToday = null;
let cachedAt = 0;
const dbToday = async () => {
  const now = Date.now();
  if (cachedDbToday && now - cachedAt < 60_000) return cachedDbToday;
  const [[row]] = await db.query(`SELECT DATE_FORMAT(NOW(), '%Y-%m-%d') today`);
  cachedDbToday = row.today;
  cachedAt = now;
  return cachedDbToday;
};
const parseYMD = (str) => {
