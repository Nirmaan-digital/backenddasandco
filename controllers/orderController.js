...
const editOrder=async(req,res)=>{try{const old=await getOrderById(req.params.id);if(!old)return res.status(404).json({success:false,message:"Order not found"});const f=await resolveOrderFields(req.body,old);const status=req.body.status==="Completed"?"Delivered":(req.body.status||old.status||"Pending");await updateOrder(req.params.id,f.clientId,f.orderNumber,f.ornamentName,f.weight,f.stoneWeight,f.netGoldWeight,f.percentage,f.labourCharge,f.goldEarned,f.deliveryDate,status,f.notes,f.category,old.status);await processDeliveredOrderGold(await getOrderById(req.params.id));res.json({success:true,message:"Order updated successfully",order:await getOrderById(req.params.id)});}catch(e){console.error(e);res.status(500).json({success:false,message:e.message});}};
